...
exports.followingProjectList = async (page, limit, usrid = 0) => {
  try {
    ...
    (SELECT COUNT(pu.project_id) FROM project_upvotes pu WHERE pu.project_id = p.id) as upvotes 
    FROM projects p
    LEFT JOIN user_following uf ON p.user_id = uf.following_user_id 
...
exports.projectById = async (projectId, userId) => {
  try {
    ...
    let sqlQuery = `SELECT p.id,p.topic,SUBSTRING_INDEX(p.description, ' ', 100) as description,u.name,u.username,p.skills,p.user_id,
      GROUP_CONCAT(d.id) departments,GROUP_CONCAT(d.name) as departments_name,p.created_at,
      (SELECT COUNT(pu.project_id) as count FROM project_upvotes pu WHERE pu.project_id IN (SELECT pro.id FROM projects pro WHERE pro.user_id = p.user_id)) as totalReputation,
      u.level,
