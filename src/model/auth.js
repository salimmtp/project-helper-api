...
exports.register = async data => {
  ...
  try {
    ...
    const [userId] = await conn.query(`INSERT INTO users SET ?;`, [
      {
        name: data.name,
        email: data.email,
    ...
    let departmentInsert = data.department.map(e => {
      return [userId.insertId, e];
    });
    await conn.query('INSERT INTO user_department (user_id, department_id) VALUES ?;', [departmentInsert]);
...
exports.verifyEmail = email => db.query(`SELECT id,name,email,username,password,level FROM users WHERE email=?;`, [email]);
